...
box[] cuBe = new box[250]; // change it so 0 follows mouse, 1 follows 0, 2 -> 1, 3 -> 2, ... 2500 -> 2499
...
float inc = 48; // speed towards destination
int r = 42;
int g = 160;
int b = 200;
...
void draw() {
  ...
  for (int i = 0; i < cuBe.length; i++) {
    if (i > 0) {
      cuBe[i].xd = cuBe[i-1].x;
      cuBe[i].yd = cuBe[i-1].y;
    }
    cuBe[i].display();
    cuBe[i].move();
    cuBe[i].gravitate();
