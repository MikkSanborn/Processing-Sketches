  ...
  if (drawConstruction || showAll) {
    for (int i = 0; i < count; i++) {
      if (i > 0) {
        ...
        strokeWeight(3);
        rect(-r[i]+x[i-1], -r2[i]+y[i-1], r[i]*2, r2[i]*2);
        ...
        stroke(255, 100, 180);
        strokeWeight(3);
        rect(width/2-r[i], height/2-r2[i], r[i]*2, r2[i]*2);
...
void keyReleased() {
  if (key == ' ') {
    ...
    for (int i = 0; i < count; i++) {
      ...
      r2[i] = (int) random(minRe, maxRe)*reMult; // was '/reReduction' instead of '*reMult'
...
void calculatePos(int i) {
  if (theta[i] > (PI/4) && theta[i] < (3*PI/4)) { // on top
    ...
    y[i] = r2[i]+(i>0 ? y[i-1]:height/2);
  } else if (theta[i] >= (3*PI/4) && theta[i] < (5*PI/4)) {
    float theta2 = theta[i]-(3*PI/4);
    x[i] = -r[i]+(i>0 ? x[i-1]:width/2);
    y[i] = r2[i]-((r2[i]*sqrt(2)*sin(theta2))/sin((3*PI/4)-theta2))+(i>0 ? y[i-1]:height/2);
